...
/* harmony export */   "ie": () => (/* binding */ dateOrTimeFormat)
/* harmony export */ });
/* unused harmony export playTimeConverter */
/**
 * 포멧 형식
 * 년: YY or YYYY
 * 월: M or MM
 ...
 */ const dateFormat = (value, format, separator)=>{
 ...
 */ const dateOrTimeFormat = (value, format)=>{
    // 일주일 이후
    if (Date.now() - new Date(value).getTime() > 1000 * 60 * 60 * 24 * 7) return dateFormat(value, format);
    else return timeFormat(value);
};
// 플레이 시간 변환기
const playTimeConverter = (duration)=>{
    if (+duration >= 60) {
 ...
 */ const minuteFormat = (date, format)=>{
    ...
    // "mm"일 때 ( 06 )
    if ((result === null || result === void 0 ? void 0 : result.length) === 2 && minute < 10) {
        minute = `0${minute}`;
    }
    // "m"일 때 ( 6 )
    return minute;
 ...
 */ const secondFormat = (date, format)=>{
    ...
    // "ss"일 때 ( 06 )
    if ((result === null || result === void 0 ? void 0 : result.length) === 2 && second < 10) {
        second = `0${second}`;
    }
    // "s"일 때 ( 6 )
    return second;
